...
opt = {'data_grid' : data_grid}

bragg_filter = Q_Mean_Sigma_Cart(opt,comm)


n = 20
datasets = [np.linalg.norm(data_grid,axis = -1) for i in range(n)]
...
for i,center in enumerate(bragg_centers):
    data = datasets[i]
    data[center[0],center[1]-1:center[1]+2,center[2]-1:center[2]+2] = 100.0
...
#p.show(datasets)
result = bragg_filter.count_bragg_pixels(np.array(datasets),bragg_filter.radial_masks,3)
