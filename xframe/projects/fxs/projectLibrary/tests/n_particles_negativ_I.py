...
scales = np.linspace(0.2,4,n_points)
...
def eval_diff_neg_volume(scale):
    ...
    I_lm[0]/=scale
    I = cht.inverse(I_lm)
    neg_volume = integrator.integrate(I.real<0)
    return neg_volume

def scan_n_particles_new(scales,dataset_name,scan_range,radius,shift,q_min_id,**kwargs):
    neg_fractions=[]
    for scale in scales:
        I_lm = [ np.zeros((nq, 2*o+1),dtype=complex) for o in range(max_order+1)]
        for i in range(len(I_lm)):
            ...
            I_l[:,:p_matrix.shape[-1]]=p_matrix
        I_lm[1:] = [lm/shift for lm in I_lm[1:]]
        I_lm[0]= a_int[:,None]/scale
        #I_lm[0]/=scale
        I = cht.inverse(I_lm)
        I[:q_min_id]=0
        #I2[:q_min_id]=0
        I_neg = np.where(I<0,I,0)*-1
        neg_volume = integrator.integrate(I_neg>0)
        #I_neg = np.copy(I.real)
        #I_neg[I.real>=0]=0
        #neg_int = integrator.integrate(-1*I_neg)
        neg_fractions.append(neg_volume)
    return np.array(neg_fractions)

def scan_n_particles_new2(scales,**kwargs):    
    I_lm = [ np.zeros((nq, 2*o+1),dtype=complex) for o in range(max_order+1)]
    for i in range(len(I_lm)):        
        I_l = I_lm[i]
...
def scan_n_particles_new3(scales,**kwargs):
    I_lm = [ np.zeros((nq, 2*o+1),dtype=complex) for o in range(max_order+1)]
    for i in range(len(I_lm)):        
        I_l = I_lm[i]
        I_l[:,:p_matrices[i].shape[-1]]=p_matrices[i]

    I=cht.inverse([a_int[:,None]]+I_lm[1:])
    I00 = np.abs(p_matrices[0].flatten())
    I00y00=I00/(2*np.sqrt(np.pi))
    
    neg_volume = np.array(tuple(integrator.integrate((I+(1/s-1)*I00y00[:,None,None])<0) for s in scales))
    
    return neg_volume
#locals().update(scan_n_particles_new2())

neg = Multiprocessing.comm_module.request_mp_evaluation(scan_n_particles_new3,argArrays = [scales],callWithMultipleArguments=True,n_processes = 40,splitMode='modulus')
#q_min_id=0
