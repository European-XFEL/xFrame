...
from xframe.analysis.interfaces import RecipeInterface
from xframe.analysis.analysisWorker import AnalysisWorker


from .analysisLibrary.hankel_transforms import generate_ht
from .analysisLibrary.hankel_transforms import generate_weightDict

from .analysisLibrary.ft_grid_pairs import get_grid
from .analysisLibrary.ft_grid_pairs import polarFTGridPair_PNAS
from .analysisLibrary.ft_grid_pairs import radial_grid_func_zernike
from .analysisLibrary.ft_grid_pairs import spherical_ft_grid_pair_zernike
from .analysisLibrary.ft_grid_pairs import polar_ft_grid_pair_zernike
from .analysisLibrary.ft_grid_pairs import polarFTGridPair_SinCos_new
from .analysisLibrary.fourier_transforms import generate_ft
from .analysisLibrary.misk import getAnalysisRecipeFacotry
from .analysisLibrary.misk import get_analysis_process_factory
from .analysisLibrary.misk import generate_calc_center
from .analysisLibrary.misk import generate_absolute_value
from .analysisLibrary.misk import generate_square
from .analysisLibrary.misk import _get_reciprocity_coefficient
from .analysisLibrary.misk import save_to_dict
from .analysisLibrary.misk import copy as copy_operation
from .analysisLibrary.misk import diff as diff_operation
from .analysisLibrary.misk import add_above_zero_index as add_above_zero_index
from .analysisLibrary.misk import load_from_dict
from .analysisLibrary.fxsSetup import generate_extract_b_coefficients
#from .analysisLibrary.fxsShrinkWrap import generateSW_SupportMaskRoutine
#from .analysisLibrary.fxsShrinkWrap import generateSW_multiply_ft_gaussian
from .analysisLibrary.fxs_Projections import ReciprocalProjection
from .analysisLibrary.fxs_Projections import ShrinkWrapParts
from .analysisLibrary.fxs_Projections import generate_negative_shift_operator,generate_shift_by_operator
from .analysisLibrary.fxs_Projections import generate_initial_support_mask
from .analysisLibrary.fxs_Projections import generate_orientation_matching
from .analysisLibrary.fxs_Projections import generate_fix_point_inversion
#from .analysisLibrary.fxs_Projections import generate_remining_SO_projection
from .analysisLibrary.fxs_Projections import RealProjection
from .analysisLibrary.fxs_invariant_tools import generate_estimate_number_of_particles_new,generate_estimate_number_of_particles_new_2
from .analysisLibrary.fxs_IO_methods import generate_error_routines
from .analysisLibrary.fxs_IO_methods import HIOProjection
from .analysisLibrary.fxs_IO_methods import generate_main_error_routine
from .analysisLibrary.fxs_IO_methods import error_reduction
from .analysisLibrary.harmonic_transforms import HarmonicTransform
from .analysisLibrary.fxs_invariant_tools import harmonic_coeff_to_deg2_invariants_2d
from .analysisLibrary.fxs_invariant_tools import harmonic_coeff_to_deg2_invariants_3d
from .analysisLibrary.fxs_invariant_tools import ccd_associated_legendre_matrices
...
class Worker(AnalysisWorker):
    def __init__(self):
        ...
        self.opt = settings.analysis
        self.process_factory=RecipeFactory({})
        MTIP.preinit()
        self.mtip = MTIP
        if settings.analysis.GPU.use:

            settings.general.n_control_workers = settings.analysis.GPU.get("n_gpu_workers",6)
            Multiprocessing.comm_module.restart_control_worker()

            ###### IMPORTANT ######
            # Due to pyOpenCL not beeing fork save one needs to querry the number of available GPUs after trying to start the GPU workers.
            # Otherwise they inherit wrong openCL instaance and could not start.
            ...
            if n_gpus == 0:
                ...
                settings.analysis.GPU.use = False
    ...
    def start_phasing(self):
        ...
        if run_profiling:
            process_id = settings.analysis.profiling.reconstruction_process_id
            if not settings.analysis.multi_process.use:
                process_id=0
    ...
    def start_parallel_phasing(self):
        ...
        mp_opt = settings.analysis.multi_process
    ...
    def post_processing(self):
        ...
        try:
            ...
            reciprocity_coefficient = _get_reciprocity_coefficient(settings.analysis.fourier_transform)
    ...
    def start_working(self):
        ...
        if opt.multi_process.use:
            ...
            self.setup_phasing_loop()
            log.info('Start Phasing: \n')
            data = self.start_phasing()
            result = np.array([data],dtype = object)
        log.info('phasing result type = {}'.format(type(result)))
...
class MTIP:
    ...
    @classmethod    
    def load_fourier_transform_weights(cls,n_radial_points,r_max):
        opt = settings.analysis
        db = database.analysis
        dimensions = opt.dimensions
    ...
    def load_mtip_data(cls):
        opt = settings.analysis
        db = database.analysis
        data = db.load('reciprocal_proj_data',path_modifiers={'structure_name':opt.structure_name,'dimensions':opt.dimensions})        
    ...
    def generate_fourier_transforms(self,harm_trf):
        ...
        #d= np.ones((settings.analysis.grid.n_radial_points,pos_orders.max()*2+1))
    ...
    def assemble_IO_variants(self):
        estimate_number_of_particles = settings.analysis.projections.reciprocal.number_of_particles.get('estimate',False)
        estimate_in_variants = settings.analysis.projections.reciprocal.number_of_particles.get('estimate_in',[])
        #use_ht_difference = settings.analysis.main_loop.get('stabilize_harmonic_transform',False)       
        ...
        for name in routine_names:
            ...
            sketch = [
                ...
                [np.array([0,0,1],dtype=np.int),[start_routine_name,'id']],
                [np.array([0,1,0],dtype=np.int),['inverse_fourier_transform','id','id']],
                [np.array([0,0,1,2],dtype=np.int),['copy','real_projection','id','id']],
    ...
    def assemble_IO_variants_old(self):
        estimate_number_of_particles = settings.analysis.projections.reciprocal.number_of_particles.estimate
        estimate_in_variants = settings.analysis.projections.reciprocal.number_of_particles.estimate_in
        if estimate_number_of_particles and ('HIO' in estimate_in_variants):
            fxs_HIO_sketch=[
                [np.array([0,1],dtype=np.int),['MTIP_start_estimate_n_particles','id']],
                [np.array([0,0,1]),['copy','id','id']],       
                [np.array([0,1,2],dtype=np.int),['id','real_support_projection','id']],
                [np.array([0,1,2],dtype=np.int),['id','other_real_projections','id']],
                [(0,1,2,0,1),['hybrid_input_output','calc_real_errors']],
                [(0,),['MTIP_end']]
            ]
        else:
            fxs_HIO_sketch=[
                [np.array([0,1],dtype=np.int),['MTIP_start','id']],
                [np.array([0,0,1]),['copy','id','id']],       
                [np.array([0,1,2],dtype=np.int),['id','real_support_projection','id']],
                [np.array([0,1,2],dtype=np.int),['id','other_real_projections','id']],
                [(0,1,2,0,1),['hybrid_input_output','calc_real_errors']],
    ...
    def assemble_output_modifier(self,opt):
        ...
        shift_center = [
            ...
            [(0,1,2),[['id',np.array([],dtype=np.object)],['id',np.array([],dtype=np.object)],['save_to_dict',np.array([self.results,'neg_center_pos','replace'],dtype=np.object)]]],
        ...
        fix_orientation=[
            ...
            [(0,1),[['id',np.array([],dtype=np.object)],['id',np.array([],dtype=np.object)],['load_from_dict',np.array([self.results,'fxs_unknowns'],dtype=np.object)]]],
        ...
        if opt.get('fix_orientation',False):
            try:
                assert settings.analysis.projections.reciprocal.SO_freedom.use,'SO Freedom is not used in reciprocal projection (projections.reciprocal.SO_freedom.use = False) therfore no orientation fixing is possible. skipping its assembly.'
    ...
    def assemble_phasing_loop(self):
        ...
        def generate_loop_method(loop_name,loop_opt,loop_number):
            ...
            hio_beta = hio_opt.beta[min(n_hio_betas-1,loop_number)]
            #log.info(f'{loop_name} : beta = {hio_beta} from betas = {hio_opt.beta}')
                    ...
                    for key in order:                    
                        ...
                        if key=='SW':
                            ...
                            for i in range(repeats):
                                ...
                                if (state['best_error'] > main_error) and (step>3):                                
                            ...
                            log.info('{} Loop:{} Method:{} Last Errors: \n{}  Best Error: {}\n number of particles = {}, max_density={}'.format(loop_name,iteration,key,errs,state['best_error'],self.rprojection.number_of_particles,np.max(new_density_pair[1])))
        ...
        def generate_output(state,iterations,initial_densities,initial_mask):
            ...
            return resultDict
        def main_loop(*args,initial_state_input={},**kwargs):
            
            initial_state = create_initial_state()
            initial_state.update(initial_state_input)
    ...
    def generate_density_guess_method(self,density_guess_specifier,real_grid):
        opt=settings.analysis.density_guess
        ...
        if isinstance(radius,bool):
            radius = settings.analysis.particle_radius
        ...
        data_type = np.dtype(complex)#settings.analysis.fourier_transform.data_type
    ...
    def generate_update_shrink_wrap(self,sw_object):        
        ...
        for lid in range(nloops): 
            ...
            sw_sigma = sw_opt.sigmas[min(n_sw_sigmas,lid)]
            ...
            sw_threshold = sw_opt.thresholds[min(n_sw_thresholds,lid)]
