...
class ProjectWorker(ProjectWorkerInterface):
    ...
    def prepare_fourier_transforms(self):
        opt = settings.project
        db = database.project
        max_order =  = opt.grid.max_order

        #make sure that all fourier transform weights are precomputed
        for bandwidth in np.arange(2,max_order+1,2):
...
class MTIP_Projection:
    def __init__(self,grid_pair):
        rp=ReciprocalProjection(grid_pair['reciprocal'],self.mtip_data,max_order)
        pass
    def project(density,stabilize_ft=False,return_error=False):
        pass
