...
class SimpleRegridder2D:
    ...
    def check_interpolation_constants(self,interpolation_constants):
        pixel_centers_match = self.pixel_centers_hash == interpolation_constants['pixel_centers_hash']
        new_grids_match = self.new_grid_hash == interpolation_constants['new_grid_hash']
        interpolation_match = self.interpolation == interpolation_constants['interpolation']
        return (new_grids_match and pixel_centers_match)
    
    def assemble_interpolation_dict(self, data: dict):
        interpolation_constants={
            'hash_pixel_center':hash(self.pixel_centers.data.tobytes()),
    ...
    def generate_apply_linear(self):
        weighs = self.interpolation_constants['weights']
...
class AgipdRegridderSimple:
    ...
    def __init__(self,geometry,new_grid_shape, interpolation = 'nearest', interpolation_constants=False):
        self.default_interpolation_constants = {
            'data':[False]*len(cart_pixel_centers),
            'mask':[False]*len(cart_pixel_centers)
        }
        ...
        print('module_boundaries.shape',np.asarray(module_boundaries).shape)
        ...
        if self.generate_new_constants:
            self.interpolation_constants = self.assemble_interpolation_constants()
    ...
    def _generate_module_masks(self,new_grid,boundaries):
        ...
        print('module masks shape',module_masks.shape)
        return module_masks
    ...
    def _generate_regridders(self,new_grid,polar_pixel_centers,module_masks):
        ...
        for m_id,mask in enumerate(module_masks):
            ...
            r = SimpleRegridder2D(module_pixel_centers,new_points,interpolation = self.interpolation,interpolation_constants = constants[m_id])
            if self.interpolation == 'linear':
                rm = SimpleRegridder2D(module_pixel_centers,new_points,interpolation = 'nearest',interpolation_constants = constants[m_id]))
            else:
                rm = r
    ...
    def _assemble_interpolation_constants(self):
        data_constants = {m_id:r.interpolation_constants for m_id,r in self.data_regridders_per_module}
        mask_constants = {m_id:r.interpolation_constants for m_id,r in self.mask_regridders_per_module}
        hash_name = self.get_interpolation_constants_name(self.geometry,self.new_grid_shape,self.interpolation)        
        interpolation_constants={'data' : data_constants,'mask':mask_constants,'hash_name'}
    def process_interpolation_constants(self,constants):
        ...
        if constants_are_valid:
            ...
            interpolation_constants = default_constants
...
ar = AgipdRegridderSimple(gg,new_grid_shape,interpolation='linear')


polar_data,polar_mask = ar.regrid(data,mask,np.arange(8))
...
database.analysis.save(base_path+'regrid_simple.vts',[polar_data[0]],grid=ar.new_grid,grid_type='polar')
