...
for max_order in max_orders:
    ...
    bl_2_0_t.append(np.abs(cross_correlation_to_deg2_invariant(new_cc,3,metadata)[0][order]).real)
    ...
    bl_2_f.append(np.abs(cross_correlation_to_deg2_invariant(cc,3,metadata)[0][order]).real)
...
layout_0_t = {'title':'$B_2$ use-odd-orders removed-cc-0-order ','x_label':'$q_1$', 'y_label':'$q_2$','title_size':40,'text_size':30}
layout_0_f = {'title':'$B_2$ removed-cc-0-order','x_label':'$q_1$', 'y_label':'$q_2$','title_size':40,'text_size':30}
layout_t = {'title':'$B_2$ use-odd-orders','x_label':'$q_1$', 'y_label':'$q_2$','title_size':40,'text_size':30}
layout_f = {'title':'$B_2$','x_label':'$q_1$', 'y_label':'$q_2$','title_size':40,'text_size':30}
layout = tuple(zip(*[[layout_f]*(n_orders+1),[layout_0_f]*(n_orders+1),[layout_t]*(n_orders+1),[layout_0_t]*(n_orders+1)]))

db.save('/gpfs/exfel/theory_group/user/berberic/MTIP/test/cc_remove_zero_component/b0_comparison.matplotlib',heat2D_multi.get_fig(tuple(zip(*[bl_2_f,bl_2_0_f,bl_2_t,bl_2_0_t])),layout=layout, grid = grid,scale = 'log',shape = [len(max_orders)+1,4],size = (50,10*(len(bl_2_f)+1)),vmin= 1e14, vmax = 1e24,cmap = 'plasma'))
